...
def create_poll(request, num_choices):
    ...
    if not membership:
        messages.error(request, "Only club leaders can create polls.")
    ...
    if request.method == "POST":
        ...
        if poll_form.is_valid() and all(cf.is_valid() for cf in choice_forms):
            ...
            for choice_form in choice_forms:
                ...
                choice.poll = poll
                choice.save()
            messages.success(request, "Poll and choices created successfully.")
            return redirect('polls:poll_list')
    else:
        poll_form = PollForm()
...
def Pollvote(request, poll_id):
    ...
    if not is_member:
        messages.error(request, "You are not authorized to vote in this poll.")
        return redirect("polls:poll_list")
...
def poll_results(request, poll_id):
    ...
    is_member = Membership.objects.filter(user=request.user, club=poll.club, membership_type__in=["leader", "member"]).exists()
    if not is_member:
        messages.error(request, "You are not authorized to view this poll.")
        return redirect("polls:poll_list")
